...
export class accessibilité {
    ...
    static accessibilité(pos: Pos, temps_en_s: number, temps_actuel_en_s:number): couple<Pos, number>[] {
        var vitesse_pied = 5;
        var vitesse_velo = 15;
        ...
        for (var stop of acc_stop_times){
            var r = accessibilité.accessibilité(stop.stop.pos,temps_en_s-(stop.arr-temps_actuel_en_s),stop.arr);
            for (c of r){
                res[res.length]=c;
            }
        }
    ...
    static compute_accessible_stops(limit_dist: number, ma_pos: Pos): couple<Stop,number>[] {
        ...
        for (var i = 0; i < tab.length; i++) {
            var d = this.distance(tab[i].lat, tab[i].long, ma_pos);
            if (d < limit_dist){
    ...
    static distance(lat: number, long: number, ma_pos: Pos): number {
        //calcule la distance entre une stations et ma position
        //retourne le résultat en km
        var R = 6378;
        var x = (lat) * R * Math.PI / 180;
        var y = (long) * R * Math.PI * Math.cos(ma_pos.lat * Math.PI / 180) / 180;
